...
接続型の指示は6段階（接続確認・取得・提案・Dry Run・実行承認・適用/検証）と共通制約に分ける。ブラウザー終了/再起動の事前承認と前面化禁止は冒頭に集約する。報告は段階別の定型とするが、実行承認に必要な操作一覧は省略しない。取得完了後に候補が0件なら適用不要として終了し、失敗や部分取得を0件と扱わない。

接続型は `preparePlan` の簡潔な入力例を持ち、添付型の計画例は維持する。移動提案はsnapshot・scope・移動元ID・移動先ID・理由だけを渡し、拡張が既存ビルダーで計画を補完する。公開スキーマは変更せず、例への実データ代入でスキーマ・Dry Runを検証する。
...
- 上部の明示コピーボタンだけが `navigator.clipboard.writeText` を呼ぶ。読み取りは禁止し、失敗時はプレビュー内の指示文を選択する。コピーのリクエスト世代・ツリー世代・本文を照合し、連打や再生成前の古い成功・失敗による通知更新とフォーカス移動を捨てる。
- 対象フォルダーが消えた場合、選択値を「見つからないフォルダー」として保持する。coreの未知スコープに対する全体フォールバックへ流さず、UIでコピー・書き出しをロックして両プロンプトを消去する。再選択で復帰し、接続型指示文も最新ツリーでscope IDが見つからなければ停止する。
- 準備状態と入力エラーはプレビューの外に表示する。入力欄の `aria-invalid` / `aria-describedby` と `role="status"` を使い、初期化・実行中・取得失敗を区別する。ネイティブの読上げ確認はREADMEの手動UX確認に残す。
- 固定ナビゲーションはURLを変更せずセクションへスクロールしてフォーカスを渡す。管理画面URLを厳密に照合するCLIを壊さず、次のTabでセクション内へ進める。
- バックアップのショートカットは読込済みの計画とDry Runを保持して書き出す。初回だけツリーを取得し、直前の再取得に失敗した既存ツリーでは保存せずエラーへ誘導する。保存後の再選択による検証は省略しない。
- 接続型とファイル型は別プロンプト。接続型は管理画面URL・拡張ID・版・セッションID・スナップショットID・対象と目的を持ち、既定は提案と相談。利用者指定のCDP接続先とラベルは認証や自動検出とみなさず、ページ内だけに保持する。接続先は認証情報なしのloopback HTTP originのみ許可。
- 対象フォルダーと空化目的は独立。最初からJSON出力を強制せず、方針相談後に計画を内部生成し、Dry Runの変更一覧への実行承認を求める。添付型は接続できない場合の代替として折りたたみに置く。
...
ページ初期化と API 公開の後、ツリーが未読込なら `loadTree({ focus: false })` を一度だけ呼ぶ。読込中の再入は既存の `state.loading` で拒否する。再読込はユーザー操作または同じコントロールを通る `refreshTree` で行い、API経由ではフォーカスを動かさない。スナップショット保存もブックマーク変更も行わない。`agent.error.noTree` はツリー未読込を示し、API の不存在を意味しない。復旧待ち journal と Apply の安全ゲートは維持する。
...
0.7.0の `preparePlan` は `features.preparePlan` で識別する。入力は `{snapshotId, scopeFolderId, moves:[{bookmarkId,destinationFolderId,reason}]}`。未知フィールド・空配列・上限超過を拒否し、最大200件なので承認前の全件表示（500行上限）を超えない。scopeは移動元を限定し、移動先はプロファイル全体の既存フォルダーから選ぶ。snapshotは別セッション・再読込で失効するが、ブラウザー外部変更の即時検知を意味しない。

`buildPlanFromProposal` は移動を1件ずつ既存ビルダーで生成し、全体を既存Dry Runへ渡す。重複や親子選択を間引かず、全行movableの場合だけ共有 `loadPlan` と `renderDryRun` へ進む。no-opも全体拒否。未知境界同士の一致を認めない。同期的な入力拒否は既存計画を保持する。非同期読込後はsnapshot・scope・planSeqを再照合し、古い提案が自分の計画として残った場合だけ撤回する。置換された別計画は消さない。読込開始時に旧Dry Runと承認を失効させる。生成後はファイル計画と同じ既存の再照合・承認・backup・journal・rollback経路に従う。件数は既存Dry Runが算出し、生成計画の重複返却は行わない。

表示名は `Splendid Bookmarks for AI Agents`。拡張ID・package/repo名・保存キー・公開API名は維持し、AI内蔵や権限境界を示唆しない。既存Playwright CLI / MCP接続を優先し、必要な場合にCDPを利用する。新たな通信基盤・認証・権限・依存は追加しない。

接続型の追加機能は `capabilities.state.features` で発見する。検索・ページ送りを元のentriesで確定してから `buildAgentTreeRows(entries, page.shown)` で返却分だけを変換する。親を含む完全ツリーの索引で境界を解決し、ページ外の恒久ルートを失わない。索引構築と検索は引き続き全体を走査するため、全計算量がページ件数だけになるとは主張しない。Dry RunはUIと同じ結果を構造化して返す。`apply` の任意 `planDigest` は承認した計画のすり替わりを拒否する補助で、既存クライアントの引数省略を維持する。実行前の承認はエージェント運用上の契約であり、この入力だけを強制的な権限境界としない。

API行とコンテキストJSONの境界値は共通の `entryBoundary` で解決する。同期属性が欠落するか子と恒久ルートが不一致ならnullを返し、ファイル用指示文でもnull同士を同一保存先と扱わない。`tests/agent-pagination.test.js` は実ハンドラーを隔離実行し、全ページの返却IDに対して行生成が1回ずつであること、境界・順序・ロック・古いcursor拒否が維持されることを検証する。CLIは取得前の統計/セッション不一致を拒否し、空タイトルは正当なデータとして保持する。

`refreshTree` は世代更新だけで成功とせず `treeLoadFailed` も照合し、取得後の画面構築失敗を `agent.error.refreshFailed` にする。`loadPlan` は自身が開始した `planSeq` と終了時の世代を照合し、UIの別読込や手動計画に置き換えられた場合は `agent.error.superseded` を返す。API内の逐次実行だけではUI由来の置き換えを防げないため、この照合が必要。

プロンプトのコード例は管理画面内の実行コンテキストで動く `run` の定義を含め、応答の `state` 階層と業務結果の判定を明示する。`capabilities.state.limits.maxBatchOperations` は実行コントローラーと同じ定数から公開し、計画の入力上限と混同しない。未知の境界や消失した対象は引き続き停止対象とする。
