...
function selfTest() {
  ...
  for (const newline of ["\n", "\r\n", "\r"]) {
    assert.deepEqual(
      extractStoreListings(fixture.replaceAll("\n", newline)),
      expected,
    );
  }
  ...
  console.log(
    "SELF-TEST PASS: field identity, malformed input, newline equivalence and content mismatches",
  );
}
...
try {
  ...
  if (values.help) {
    ...
    if (values.locale || values.field || values.actual)
      throw new Error("--self-test cannot include comparison options");
    selfTest();
  } else {
    if (values.locale !== undefined && !["en", "ja"].includes(values.locale))
      throw new Error("Locale must be en or ja");
    if (
      values.field !== undefined &&
      !["summary", "description"].includes(values.field)
    )
      throw new Error("Field must be summary or description");
    if (
      values.actual !== undefined &&
      (!values.actual.trim() || !values.locale || !values.field)
    ) {
      throw new Error(
        "--actual requires a nonempty path and explicit --locale and --field",
      );
    }
    const listings = extractStoreListings(
      readFileSync(new URL("../docs/cws-listing.md", import.meta.url), "utf8"),
    );
    const fields = [];
    ...
    if (values.actual === undefined) {
      ...
      const matches = listingCopyMatches(
        listings[values.locale][values.field],
        actual,
      );
      console.log(
        JSON.stringify(
          {
            ...
            actual: {
              ...
              sha256: createHash("sha256").update(actual, "utf8").digest("hex"),
            },
          },
          null,
          2,
